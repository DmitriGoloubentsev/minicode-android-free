...
1. Go to [Google Play Console](https://play.google.com/console) and pay $25 registration fee
2. Complete identity verification (may take several days)
3. Create two app listings:
   - **MiniCode** (`com.minicode`) — Paid $8.99
   - **MiniCode Free** (`com.minicode.free`) — Free
4. For each app, fill in the store listing:
   - **Title, descriptions**: use content from `fastlane/metadata/android/en-US/`
   - **Icon**: `fastlane/metadata/android/en-US/images/icon.png` (512x512)
   - **Screenshots**: from `fastlane/metadata/android/en-US/images/phoneScreenshots/`
   - **Feature graphic**: 1024x500 banner image (required, create one)
   - **Category**: Tools or Productivity
5. Complete the **content rating** questionnaire
6. Set **privacy policy URL**: `https://minicode.app/privacy`
7. Set up **app signing**: opt into Google Play App Signing (recommended) or manage your own key

### Building signed APKs

```bash
export JAVA_HOME=/opt/android-studio/jbr
export ANDROID_HOME=/opt/dimach/Android/Sdk

# Copy notfree sources (if not already present)
cp -a ../minicode-android-notfree/app/src/play app/src/
cp -a ../minicode-android-notfree/app/src/playFree app/src/
cp -a ../minicode-android-notfree/app/libs app/
...
# Verify
$APKSIGNER verify --print-certs minicode-play.apk
$APKSIGNER verify --print-certs minicode-playFree.apk
```

### Uploading a release

1. Go to [Google Play Console](https://play.google.com/console) → select app
2. **Release** → **Production** → **Create new release**
3. Upload the signed APK (`minicode-play.apk` or `minicode-playFree.apk`)
4. Add release notes (from `fastlane/metadata/android/en-US/changelogs/<versionCode>.txt`)
5. **Review and roll out** to production

### Upload to minicode.app

```bash
cd ..  # back to parent repo
./upload.sh apk       # upload APKs to minicode.app/minicode/releases/
./upload.sh update    # upload minicode.txt for in-app update checker
./upload.sh website   # upload landing page + privacy policy
./upload.sh all       # all of the above + speech model
```
